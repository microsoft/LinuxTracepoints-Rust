...
/// Options for the "info" suffix generated by the `json_info_display` method of
/// [`EventHeaderEventInfo`].
#[repr(C)]
#[derive(Clone, Copy, Debug, Default, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct PerfInfoOptions(u32);

#[allow(non_upper_case_globals)]
impl PerfInfoOptions {
    /// Returns a `PerfInfoOptions` with the specified numeric value.
    pub const fn from_int(value: u32) -> Self {
        return Self(value);
    }

    /// Returns the numeric value corresponding to this `PerfInfoOptions` value.
    pub const fn as_int(self) -> u32 {
        return self.0;
    }
    ...
    /// No flags set (disables the "info" suffix).
    pub const None: Self = Self(0);

    /// Event identity, `"n":"provider:event"` before the user fields (not in the suffix).
    /// This flag is not used by `json_info_display`, but may be used by the caller to
